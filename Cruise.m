...
rho5static = rho(5)/(1+(gamma_turb -1)/2  ) ^(1/(gamma_turb-1));
T5staticto =T(5)/(1+(gamma_turb -1)/2  ); % again assuming ma =1
c_5 = sqrt(Rideal*T5staticto*gamma_turb);
A5 = mcore/(rho5static*c_5);
A9 = mcore/(rho(8)*Uexitcore);
A1 = mtotal/(rhoa*Ua);
...
Mcoreto = A5*c_5to*rho5to; %mc questionable value maybe should be higher
Mbypassto = mtotalto-Mcoreto;
Bto = Mbypassto/Mcoreto;

Tto(7) = Tto(6)-(Cp_comp/Cp_turb)*( (Tto(3)-Tto(2)) + (Bto+1)*(Tto(2)-Tto(1))  )
...
rho8 = Pa2/(Rideal*Tto(8))
 mcoretakeofftest = Uexitcoretakeoff * rho8*A9
