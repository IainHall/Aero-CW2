...
T_03t = (Cpg./Cp).*(t_hpt).*(T_04./T_02).*(T_02t)+T_023t; 
...
T_4t = T_04t./(1+((yg-1)./2));
P_4t = P_04t./((T_04t./T_4t).^((yg./yg-1)));
%P_04t = P_4t.*(1+((yg-1)./2)).^(yg./(yg-1));
V_4t = (yg.*R.*T_4t).^(0.5);
rho_4t = P_4t./(0.287.*T_4t);
Mc4t = rho_4t.*A_4.*V_4t;

T_045t = T_04t.*(t_hpt);
P_045t = P_04t*(T_045t/T_04t)^(yg/(Ec*(yg-1)));
...
if ratioH > ratioG
    ...
    disp('not choked flow')
    P_9t=P_2t;
    T_9t=T_05t.*(P_9t./P_05t).^((yg-1)./(yg.*Ec));
    
    M_9t =1;
    %M_9t = ((2./(yg-1)).*((P_05t./P_9t).^((yg-1)./yg))-1).^0.5;
    %T_9 = ((V_19./M_9).^2).*yg.*R;
    V_9t = sqrt(Cpg*(T_05t-T_9t))
    %V_9t=M_9t.*(yg.*R.*T_9t).^0.5;
     
    rho_9t = P_9t./(0.287.*T_9t);
    diff4 = abs(V_9t-V_19t)
end 
...
pi_b = P_023t./P_02t;
pi_c = (((T_02t./T_023t).^((y-1)./(y.*Ec))).*FPR2.*pi_b).^0.5;
